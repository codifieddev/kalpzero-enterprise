...
def _serialize_itinerary_day(model) -> dict[str, object]:
    return {
        ...
        "hotel_ref_id": model.hotel_ref_id,
        "activity_ref_ids": model.activity_ref_ids,
        "transfer_ref_ids": model.transfer_ref_ids,
    }
...
def _serialize_package(model, itinerary_by_package: dict[str, list[dict[str, object]]]) -> dict[str, object]:
    return {
        ...
        "status": model.status,
        "itinerary_days": itinerary_by_package.get(model.id, []),
...
def _serialize_lead(model) -> dict[str, object]:
    return {
        ...
        "tenant_id": model.tenant_id,
        "source": model.source,
        "interested_package_id": model.interested_package_id,
        "departure_id": model.departure_id,
...
def _build_itinerary_map(itinerary_days: list) -> dict[str, list[dict[str, object]]]:
    ...
    for item in itinerary_days:
        itinerary_by_package[item.package_id].append(_serialize_itinerary_day(item))
...
def create_departure(
    ...
    *,
    ...
    _audit(
        ...
        subject_id=departure.id,
        metadata={"package_id": package_id, "status": effective_status},
    )
    db.commit()
